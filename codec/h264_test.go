...
func TestPacketListFromH264Mode0(t *testing.T) {
	...
	pl := codec.PacketListFromH264Mode0(joinNals([][]byte{nal2000, nal3000}), 100)
...
func TestPacketListFromH264Mode1(t *testing.T) {
	...
	pl := codec.PacketListFromH264Mode1(joinNals([][]byte{nal500, nal495}), 100, 1000)
	if pl.Len() != 1 && len(pl.Payload) != 1000 {
		t.Fatal("should be packed as a stapA")
	}
	pl = codec.PacketListFromH264Mode1(joinNals([][]byte{nal500, nal496}), 100, 1000)
	if pl.Len() != 2 && len(pl.Payload) != 500 && len(pl.Next.Payload) != 496 {
		t.Fatal("should be packed as 2 nals")
	}
	pl = codec.PacketListFromH264Mode1(nal1000, 100, 1000)
	if pl.Len() != 1 && len(pl.Payload) != 1000 {
		t.Fatal("should be packed as a single nal")
	}
	pl = codec.PacketListFromH264Mode1(nal1001, 100, 1000)
	ppl := pl.Next
	if pl.Len() != 2 &&
	...
	pl = codec.PacketListFromH264Mode1(nals, 100, 1000)
	if pl.Len() != 5 {
